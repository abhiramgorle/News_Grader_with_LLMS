...
class PredictionProcessor:
    ...
    def extract_predictions_with_retry(self, article_text, graded_examples=None, max_retries=3):
        ...
        if graded_examples:
            ...
            for i, ex in enumerate(graded_examples, 1):
                ...
                examples_text += f"Rejected PREDICTIONS (only {ex['count']} were marked as false predictions):\n"
                for j, pred in enumerate(ex['rejected_predictions'], 1):
                    examples_text += f"{j}. {pred}\n"
                examples_text += "\n" + "-" * 80 + "\n"
        
        system_prompt = f"""
    You are a News Prediction Extractor. You must learn from the examples below to understand what the user considers a valid prediction.When in doubt, INCLUDE the prediction. It is better to over-extract than miss valid predictions.
    ...
    - Fears or concerns that have no directional future claim whatsoever
    ...
    def estimate_deadline_with_retry(self, prediction_text, prediction_context, article_date, max_retries=3):
        ...
        - NOT be: clearly fictional statements, article metadata, or pure editorial opinion with no future claim

        CRITICAL: Respond ONLY with valid JSON:
        {
        "is_prediction": "YES" | "NO",
        "deadline_estimate": "YYYY-MM-DD" | "UNKNOWN",
        "deadline_confidence": 1-5 (1=no timeframe possible, 2=rough timeframe within 2 years, 3=within specific year, 4=within quarter, 5=explicit date given),
    ...
    def load_graded_examples_full_context(self, graded_file_path, num_examples=455):
        ...
        try:
            ...
            all_groups = graded_df.groupby('Article_Number')
            ...
            for article_num, group in list(article_groups)[:num_examples]:
                ...
                article_rejected = rejected_all[rejected_all['Article_Number'] == article_num]['Prediction'].tolist()
                
                examples.append({
                    'article_text': article_text,
                    'validated_predictions': validated_predictions,
                    'rejected_predictions': article_rejected[:3],
